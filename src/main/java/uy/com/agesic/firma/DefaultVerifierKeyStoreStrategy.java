...
public class DefaultVerifierKeyStoreStrategy {
	...
	public DefaultVerifierKeyStoreStrategy(String certificateAuthorityTrustStorePath,
			String certificateAuthorityTrustStorePassword, String certificateAuthorityTrustStoreAlias,
			String certificateAuthorityRevocationsPath, String signerCertificateFromCAPath, boolean checkNonRepudiation,
			Date date) {
	...
	private void verifyX509Certificate(X509Certificate signerCertificate) throws DocumentSignException {
		...
		try {
			////////// TODO : TrustStore
			if (usesTrustStore) {
				caCert = getCertificateFromKeyStore(certificateAuthorityPath, certificateAuthorityTrustStorePassword,
						certificateAuthorityTrustStoreAlias);
			} else {
		...
		if (certFromCA == null || equals) {
			/*
			 * <pre> KeyUsage ::= BIT STRING { digitalSignature (0),
			...
			// El certificado embebido en el documento no es el mismo que el
			// dado por la Autoridad Certificadora
			throw new DocumentSignException("2");
		}
	}

	/**
	 * The method checks if a given cetificate is revoked. It analyzes the
	 * revoked list of the Certificate Authority.
	 *
	 * @param certificate.
	...
	protected final Certificate getCertificateFromKeyStore(String trustStorePath, String trustStorePass,
			String trusStoreAlias) throws DocumentSignException {
		KeyStore keyStore;
		try {
